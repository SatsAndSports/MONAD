...
### Two Key Systems: Which Key Is Tweaked?

MONAD currently uses two closely related key systems derived from the same
server identity root:

- **Ed25519** is the canonical published hop identity used for QUIC pinning
- **X25519** is the derived Noise static key used in `Noise_NK_25519_...`

Blinding is applied to the **Noise hop identity**, not to the relay-to-relay
QUIC transport identity.

That means:

- relay-to-relay QUIC still authenticates the next relay using its **real**
  Ed25519-derived QUIC identity
- the nested client-to-hop Noise session authenticates the next relay using a
  **tweaked** key derived from that relay's real identity plus a tweak scalar

In practice, the client is given a tweaked public key, while the receiving hop
is told the tweak scalar so it can derive the matching tweaked private key.

### Tweak Mechanics

Let the next relay's real private scalar be `s`, real public key be `S = s*G`,
and tweak scalar be `t`.
...
Because MONAD's published hop identity is Ed25519 but Noise uses X25519, the
natural place to apply the public-key tweak is the Ed25519/Edwards form. The
client can then derive the corresponding X25519/Montgomery public key the same
way it does today for non-blinded hops.
