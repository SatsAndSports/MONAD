...
- configured-client startup and manual `recover-openings` never submit opening swaps. They finish restored or finalizing channels and cancel prepared attempts and rejected attempts without successors. `wallet export-stale-opening-inputs` requires exclusive wallet access and, after one hour plus exact empty NUT-09 restore and all-`UNSPENT` NUT-07 evidence, durably marks eligible submitted inputs `Exported` before printing bearer tokens. The proofs remain reserved. Recovery later finalizes a delayed opening, or marks exported proofs `ExternallySpent` only after exact all-`SPENT` evidence and a final empty restore. Recent, stale, clock-rollback, partial, invalid, or unavailable evidence remains reserved.
