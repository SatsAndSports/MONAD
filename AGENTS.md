...
- `SqliteClientWallet` provisions real upstream Spilman channels from loose proofs, stores MONAD channel metadata, persists ambiguous opening recovery, and uses cache-first active output-keyset selection. It refreshes the client mint cache before classifying a relay offer as stale, and separately refreshes/retries after retryable keyset mint errors. If a refreshed client cache has no active keyset accepted by a relay's stale offer, the wallet returns a dedicated stale-relay-keysets error and the session driver sends one bounded `RefreshKeysets` hint for that mint/unit before retrying from the next `SessionStatus`.
