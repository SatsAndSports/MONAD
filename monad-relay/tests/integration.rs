...
use monad_client::wallet::{
    MockWallet, MonadWallet, RelayPaymentOffer, WalletChannel, WalletChannelState, WalletError,
};
...
async fn start_relay_from_bound_listener_internal(
    ...
    let config = Arc::new(ServerConfig {
        ...
        receiver_pubkey_hex,
        trusted_mint_units,
        in_bytes_per_millisat: relay_config.pricing.in_bytes_per_millisat,
        out_bytes_per_millisat: relay_config.pricing.out_bytes_per_millisat,
        bootstrap_capabilities: None,
    ...
    wallet_manager.install_keyset_cache(mint_cache);
    wallet_manager.set_trusted_mint_units(relay_config.trusted_mint_units());
    let payments = wallet_manager.spilman_payments_for_live_with_policy(
        &relay_config.name,
...
#[tokio::test(flavor = "multi_thread")]
async fn test_relay_close_reactive_keyset_refresh_enables_new_keyset_link() {
    let mint_helper = TestMintHelper::new().await.unwrap();
    let mint = mint_helper.mint();
    let mint_listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let mint_addr = mint_listener.local_addr().unwrap();
    ...
    let old_capacity_raw = old_wallet
        .get_channel(&old_channel_id)
        .unwrap()
        .capacity_msats
        / 1000;
    let old_funded_balance_raw = old_capacity_raw / 2;
    let payment_json = old_wallet
        .build_channel_payment(&old_channel_id, &old_offer, 0, old_funded_balance_raw)
        .unwrap();
    send_control_message(
        &mut control_send,
        &ClientMessage::ChannelPayment { payment_json },
    ...
    send_control_message(&mut control_send, &ClientMessage::GetSessionStatus, false).await;
    let stale_status = expect_session_status_struct(read_control_message(&mut control_recv).await);
    let stale_advertised_sat = stale_status
        .advertisements
        .iter()
        .find(|ad| ad.mint_url == mint_url && ad.unit == "sat")
    ...
    let closed_data = payments
        .closed_data(&old_channel_id)
        .expect("closed data after reactive close");
    let receiver_proofs: Vec<cashu::nuts::Proof> =
        serde_json::from_str(&closed_data.receiver_proofs_json).unwrap();
    assert!(receiver_proofs
        .iter()
        .all(|proof| proof.keyset_id.to_string() == new_keyset_id));

    let refreshed_cache = wallet_manager.keyset_cache_snapshot();
    let refreshed_keysets = refreshed_cache
        .keysets
        .get(&mint_url)
        .expect("reactive refresh should cache the mint");
    assert!(
        !refreshed_keysets
            .get(&old_keyset_id)
            .expect("old keyset remains cached")
            .active
    );
    assert!(
        refreshed_keysets
            .get(&new_keyset_id)
            .expect("new keyset is cached")
            .active
    );
    ...
    let refreshed_status =
        expect_session_status_struct(read_control_message(&mut control_recv).await);
    let refreshed_advertised_sat = refreshed_status
        .advertisements
        .iter()
        .find(|ad| ad.mint_url == mint_url && ad.unit == "sat")
    ...
    let new_channel_id = new_wallet.pre_create_channel(1000).await.unwrap();
    assert_eq!(
        new_wallet.get_channel(&new_channel_id).unwrap().keyset_id,
        new_keyset_id
    );
    new_wallet
        .attach_channel_to_session(&new_channel_id, *conn.session_id())
        .unwrap();
    let link_json = new_wallet
        .build_link_request(&new_channel_id, &new_offer)
        .unwrap();
    send_control_message(
        &mut control_send,
        &ClientMessage::ChannelLink {
    ...
    let new_link_status =
        expect_session_status_struct(read_control_message(&mut control_recv).await);
    new_link_status.assert_linked_channel(&new_channel_id, 0, 1000, "sat");

    let new_payment_json = new_wallet
        .build_channel_payment(&new_channel_id, &new_offer, 0, 1)
        .unwrap();
    send_control_message(
        &mut control_send,
        &ClientMessage::ChannelPayment {
            payment_json: new_payment_json,
        },
        false,
    )
    .await;
...
async fn test_control_refresh_keysets_then_accepts_new_keyset_channel_link() {
    ...
    .await;
    let link_status = expect_session_status_struct(read_control_message(&mut control_recv).await);
    let new_capacity_raw = new_wallet
...
async fn test_rotated_mint_stale_relay_old_channel_ok_new_open_refused() {
    ...
    // Real client wallet backed by temp SQLite DBs.
    let temp_dir = tempfile::tempdir().unwrap();
    let loose_db_path = temp_dir.path().join("loose.db");
    let channel_db_path = temp_dir.path().join("channels.db");
    let sender_secret_hex = hex::encode([42u8; 32]);
    let loose_wallet = LooseProofWallet::open(&loose_db_path, "alice").unwrap();
    let wallet = SqliteClientWallet::open(loose_wallet, &channel_db_path, &sender_secret_hex)
    ...
    // Session 1: provision, link, and pay an old-keyset channel.
    let conn1 = connect_client_quic_secp(server_addr, &pubkey).await;
    let (mut control_send1, mut control_recv1) = conn1.open_control().await.unwrap();
    let status1 = control_handshake_status(&mut control_send1, &mut control_recv1).await;
    let advertisement1 = status1
        .advertisements
        .iter()
        .find(|ad| ad.mint_url == mint_url && ad.unit == "sat")
        .expect("relay should advertise sat keyset");
    ...
    wallet
        .attach_channel_to_session(&channel_id, *conn1.session_id())
        .unwrap();
    let link_json = wallet.build_link_request(&channel_id, &offer).unwrap();
    send_control_message(
        &mut control_send1,
    ...
    .await;
    let link_status = expect_session_status_struct(read_control_message(&mut control_recv1).await);
    let capacity_raw = link_status
        .linked_channel
        .as_ref()
        .expect("linked channel after first link")
        .capacity_raw;
    link_status.assert_linked_channel(&channel_id, 0, capacity_raw, "sat");

    let payment_json = wallet
        .build_channel_payment(&channel_id, &offer, 0, 100)
        .unwrap();
    send_control_message(
        &mut control_send1,
        &ClientMessage::ChannelPayment { payment_json },
    ...
    .await;
    let paid_status = expect_session_status_struct(read_control_message(&mut control_recv1).await);
    assert!(!paid_status.paused);

    // Rotate the mint: old keyset becomes inactive, new keyset becomes active.
    let new_keyset_id = rotate_sat_keyset(&mint, 0).await.unwrap().to_string();
    assert_ne!(old_keyset_id, new_keyset_id);

    // Force a client-side keyset refresh out-of-band: the wallet's cache lives
    // in its channel DB, so a second bridge over the same DB can refresh it.
    let refresh_bridge = SpilmanClientBridge::new(
    ...
    refresh_bridge
        ...
        .expect("client keyset refresh");
    let client_active_ids: Vec<String> = refresh_bridge
        .cached_active_keyset_ids(&mint_url, &cashu::nuts::CurrencyUnit::Sat)
        .iter()
        .map(|id| id.to_string())
        .collect();
    assert_eq!(
        client_active_ids,
        vec![new_keyset_id.clone()],
        "refreshed client cache should see only the new keyset as active"
    );

    // New loose proofs mint on the new active keyset.
    let new_proofs = mint_helper.mint_proofs(10_000).await.unwrap();
    assert!(new_proofs
        .iter()
        .all(|proof| proof.keyset_id.to_string() == new_keyset_id));
    wallet
    ...
    let holdings = wallet
        .loose_wallet()
        .list_available_proofs(&mint_url, "sat", &[])
        .unwrap();
    assert!(holdings.iter().any(|p| p.keyset_id == old_keyset_id));
    assert!(holdings.iter().any(|p| p.keyset_id == new_keyset_id));

    // End session 1 so the relay releases channel ownership.
    let _ = control_send1.send_data(Bytes::new(), true);
    drop(control_send1);
    drop(control_recv1);
    ...
    wallet.force_detach_channel(&channel_id).unwrap();
    wallet
        .attach_channel_to_session(&channel_id, *conn2.session_id())
        .unwrap();
    let link_json = wallet.build_link_request(&channel_id, &offer2).unwrap();
    let relink_status = {
        ...
        for _ in 0..50 {
            send_control_message(
                &mut control_send2,
                &ClientMessage::ChannelLink {
                    payment_json: link_json.clone(),
                },
                false,
            )
            .await;
            match read_control_message(&mut control_recv2).await {
                message @ ServerMessage::SessionStatus { .. } => {
                    status = Some(expect_session_status_struct(message));
                    break;
                }
                ServerMessage::Error { code, message } => {
                ...
                other => panic!("unexpected message during re-link: {other:?}"),
            }
        }
        status.unwrap_or_else(|| panic!("re-link never succeeded; last error: {last_error}"))
    };
    let relinked = relink_status
        .linked_channel
        .as_ref()
        .expect("linked channel after re-link");
    assert_eq!(relinked.channel_id, channel_id);
    assert_eq!(
        relinked.balance_raw, 100,
        "re-link should resume from the persisted accepted balance"
    );

    // Pay again on the re-linked old-keyset channel.
    let payment_json = wallet
        .build_channel_payment(&channel_id, &offer2, 100, 200)
        .unwrap();
    send_control_message(
        &mut control_send2,
        &ClientMessage::ChannelPayment { payment_json },
    ...
    .await;
    let paid_status2 = expect_session_status_struct(read_control_message(&mut control_recv2).await);
    assert!(!paid_status2.paused);

    // Data flows through the re-linked session.
    let mut tunnel = conn2.open_tunnel(&upper_addr.to_string()).await.unwrap();
    tunnel.write_all(b"client ahead of relay").await.unwrap();
    tunnel.shutdown().await.unwrap();
    let mut result = Vec::new();
    tunnel.read_to_end(&mut result).await.unwrap();
...
async fn test_shared_yaml_config_drives_client_socks_over_quic() {
    ...
    let mint_helper = TestMintHelper::new().await.unwrap();
    let mint_listener = TcpListener::bind("127.10.1.20:0").await.unwrap();
    let mint_addr = mint_listener.local_addr().unwrap();
    let mint_url = format!("http://127.10.1.20:{}", mint_addr.port());
    let mint_router = build_router(mint_helper.mint()).await.unwrap();
    let (mint_shutdown_tx, mint_shutdown_rx) = tokio::sync::oneshot::channel();
    tokio::spawn(async move {
        axum::serve(mint_listener, mint_router)
...
"#,
    ...
    assert_eq!(result, b"CONFIGURED CLIENT PATH");
...
struct ConfiguredSuffixRebuildCase {
    subnet: u8,
    hop_count: usize,
    failed_hop_idx: usize,
    expected_preserved_sessions: usize,
    payload: &'static [u8],
    wallet_seed: u8,
    proof_batches: usize,
    label: &'static str,
...
async fn wait_for_configured_roundtrip(
    socks_listen: SocketAddr,
    upper_addr: SocketAddr,
    payload: &[u8],
    context: &str,
    ...
    for _ in 0..80 {
        ...
        .await
        {
            Ok(Ok(result)) if result == expected => return result,
    ...
    panic!("{context} should succeed: {last_error:?}");
}

fn assert_suffix_rebuild_channel_invariants(
    initial_channels: &[WalletChannel],
    rebuilt_channels: &[WalletChannel],
    expected_preserved_sessions: usize,
    expected_rebuilt_suffix_sessions: usize,
    context: &str,
) {
    ...
    assert_eq!(
        ...
        "{context}: suffix rebuild should reuse existing channel records without provisioning new ones"
    );
    ...
    for initial in initial_channels {
        let Some(rebuilt) = rebuilt_channels
            ...
            panic!(
                "{context}: channel {} should be reused after suffix rebuild",
                initial.channel_id
            );
        };
            ...
            && initial.attached_session_id.is_some()
        {
            ...
            assert_eq!(
                ...
                "{context}: preserved prefix channel should remain open"
            );
        } else {
    ...
    assert_eq!(
        preserved_sessions, expected_preserved_sessions,
        "{context}: unexpected number of preserved prefix sessions"
    );
    assert_eq!(
        rebuilt_suffix_sessions, expected_rebuilt_suffix_sessions,
        "{context}: unexpected number of rebuilt suffix sessions"
    );
}
...
async fn test_configured_client_rebuilds_suffix_after_final_hop_restart() {
    run_configured_suffix_rebuild_case(ConfiguredSuffixRebuildCase {
        subnet: 7,
        hop_count: 2,
        failed_hop_idx: 1,
        expected_preserved_sessions: 1,
        payload: b"suffix final",
        wallet_seed: 37,
        proof_batches: 3,
        label: "suffix-final-e2e",
    })
    .await;
}
...
async fn test_configured_client_rebuilds_suffix_after_middle_hop_restart() {
    run_configured_suffix_rebuild_case(ConfiguredSuffixRebuildCase {
        ...
        failed_hop_idx: 1,
        expected_preserved_sessions: 1,
        payload: b"suffix middle",
        wallet_seed: 41,
        proof_batches: 5,
        label: "suffix-middle-e2e",
    })
    .await;
}

#[tokio::test(flavor = "multi_thread")]
async fn test_configured_client_rebuilds_three_hop_suffix_in_five_hop_route() {
    run_configured_suffix_rebuild_case(ConfiguredSuffixRebuildCase {
        subnet: 9,
        hop_count: 5,
        failed_hop_idx: 2,
        expected_preserved_sessions: 2,
        payload: b"suffix five hop",
        wallet_seed: 43,
        proof_batches: 8,
...
async fn test_shared_yaml_config_drives_two_hop_client_socks_over_quic() {
    ...
    let mint_helper = TestMintHelper::new().await.unwrap();
    let mint_listener = TcpListener::bind("127.10.2.20:0").await.unwrap();
    let mint_addr = mint_listener.local_addr().unwrap();
    let mint_url = format!("http://127.10.2.20:{}", mint_addr.port());
    let mint_router = build_router(mint_helper.mint()).await.unwrap();
    let (mint_shutdown_tx, mint_shutdown_rx) = tokio::sync::oneshot::channel();
    tokio::spawn(async move {
        axum::serve(mint_listener, mint_router)
    ...
    let transport_key_2 = SecpTransportKeypair::generate();
...
"#,
        ...
        channel_db_path.display(),
        sender_secret_hex,
        receiver_secret_1.to_secret_hex(),
        hex::encode(quic_cert_1.seed()),
        hex::encode(transport_key_1.normalized_secret_bytes()),
        relay_1_listen,
        mint_url,
        receiver_secret_2.to_secret_hex(),
        hex::encode(quic_cert_2.seed()),
        hex::encode(transport_key_2.normalized_secret_bytes()),
        relay_2_listen,
        mint_url,
        socks_listen,
        relay_1_listen,
    ...
    let mut result = Vec::new();
    stream.read_to_end(&mut result).await.unwrap();
    assert_eq!(result, b"CONFIGURED TWO HOP PATH");
    ...
    relay_2_handle.await.unwrap().unwrap();
    let _ = mint_shutdown_tx.send(());
}
